...
import { drafLabelsAction, rejectedLabelsAction } from '../../redux/actions/labelActions';
...
const RejectedLabels = () => {
  ...
  const {rejectedLabels} = useSelector(state => state)
  const {rejectedLabelsRequest, rejectedLabelsSuccess, rejectedLabelsFail, rejectedLabel} =rejectedLabels;

  const dispatch = useDispatch()
  useEffect(() => {
      dispatch(rejectedLabelsAction({companyId, productId, createdBy: userId},token))
  }, [])
  useEffect(() => {
      if(rejectedLabelsSuccess){
        setData(rejectedLabel)
      }
      if(rejectedLabelsFail && rejectedLabelsFail.message != "doesn't exist any label!"){
          toast.error(`${rejectedLabelsFail.message}`)
      }
  }, [rejectedLabelsSuccess, rejectedLabelsFail])
    ...
    <div style={{height:'70vh', width:'100%', display:'flex'}}>
      ...
      <main style={{paddingTop:'0px', width:'100%'}}>
        ...
        <section className='container' style={{marginTop:'20px', padding:'10px'}}>
            <h6>Rejected Label: {data.length} </h6>
            <div className="table-responsive">
                ...
                {!rejectedLabelsRequest && data.length < 1 &&
                    <p style={{width:'100%', marginTop:'20px', display:'flex', justifyContent:'center'}}>
                        we didn't find any Rejected project!
                </p>
                }
            </div>
        </section>
