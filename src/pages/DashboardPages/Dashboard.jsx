...
import { Print } from "@mui/icons-material"; // Import other icons as needed
...
const Dashboard = () => {
  ...
  const {
    ...
    dashboardCompanySuccess,
    dashboardCompanyFail,
  ...
  const groupedImages = companyData?.companyDocuments.reduce(
    (acc, _, index, array) => {
  ...
  const DummyData = [
    {
      icon: <TaskOutlinedIcon style={{ fontSize: "50px", color: "#ff9800" }} />,
      title: "Ongoing Labels",
      count: dashboardCardData.onGoingLabels,
      color: "#BE7609",
    },
    {
      icon: (
        <CategoryOutlinedIcon style={{ fontSize: "50px", color: "#4caf50" }} />
      ),
      title: "Received Labels",
      count: dashboardCardData.productCount,
      color: "#207D23",
    },
    {
      icon: (
        <LabelOutlinedIcon style={{ fontSize: "50px", color: "#2196f3" }} />
      ),
      title: "Total Labels",
      count: dashboardCardData.labelCount,
      color: "#062D60",
    },
    {
      icon: <Print style={{ fontSize: "50px", color: "#f44336" }} />,
      title: "Total Order Print",
      count: dashboardCardData.totalPrintCount,
      color: "#AC190E",
    },
  ];
  ...
  const fetchChartData = async () => {
    ...
    try {
      ...
      const response = await axios.get(
        `${process.env.REACT_APP_BASE_URL}/api/v1/label/dashboard-data/${companyId}/${userId}`,
        config
      );
  ...
  return (
    ...
    >
      ...
      <main
        className=""
        style={{ paddingBottom: "0px", width: "100%", overflow: "hidden" }}
      >
        {/* Dashboard header  */}
        ...
        <div
          className=""
          style={{ paddingBottom: "50px", backgroundColor: "" }}
        >
          <section className="chart-section container mt-4">
            {recentOrder?.length > 0 && (
              <div className="mb-3">
                {
                  <h3
                    style={{ color: "#062d60", fontWeight: "700", margin: "0" }}
                  >
                    {!loading && chartData
                      ? recentOrder[0]?.companyId?.companyName
                      : "..."}
                  </h3>
                }
                {
                  ...
                  >
                    {!loading && chartData
                      ? recentOrder[0]?.companyId?.companyAddress +
                        " " +
                        recentOrder[0]?.companyId?.companyCity +
                        " " +
                        recentOrder[0]?.companyId?.companyCountry
                      : "..."}
                  </p>
                }
              </div>
