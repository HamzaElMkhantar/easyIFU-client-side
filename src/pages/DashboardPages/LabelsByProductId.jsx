...
const LabelsByProject = () => {
  ...
  const handleIntendedPurpose = (item) => {
    ...
    const isItemSelected = intendedData.some(
      (dataItem) => dataItem._id === item._id
    );

    if (isItemSelected) {
      // If item exists in intendedData, remove it
      const updatedData = intendedData.filter(
        (dataItem) => dataItem._id !== item._id
      );
  ...
  const { createLabelRequest, createLabelSuccess, createLabelFail, label } =
    createLabel;
  const { deleteProjectRequest, deleteProjectSuccess, deleteProjectFail } =
    deleteProject;
  const { logoutRequest, logoutSuccess, logoutFail } = logout;
  const {
    archiveToggleProjectRequest,
    archiveToggleProjectSuccess,
    archiveToggleProjectFail,
    archiveToggleData,
  } = archivedProjectToggle;
  const {
    duplicateProjectRequest,
    duplicateProjectSuccess,
    duplicateProjectFail,
    deplicatedProject,
  } = duplicateProject;
  ...
  return (
    ...
    >
      ...
      <main className="" style={{ paddingTop: "0px", width: "100%" }}>
        ...
        <Heading><Link className='linkStyle' to='/dashboard/project'>Projects</Link> / {allProjects.length > 0 &&<Link className='linkStyle' to={'/dashboard/products/' + allProjects[0]?.productId?.projectId}>Products</Link>} / </Heading>
        ...
        <section className="container pb-5" style={{ marginTop: "15px" }}>
                ...
                <form onSubmit={handleSubmit}>
                  ...
                  <div className="form-field">
                    <label htmlFor="service">
                      3- choose the intended purpose for this label:
                    </label>
          ...
          >
            <Box sx={style} style={{ backgroundColor: "#FFD631", border: "0" }}>
              ...
              >
                Please be aware that updating certain information will require
                creating a new product with a new Unique
                <br />
                Device Identification (UDI) code. Changes have
                <br />
                been made to the following aspects :
              </h5>
              <ul>
              ...
              <Typography id="modal-modal-description" sx={{ mt: 2 }}>
                {!duplicateProjectRequest ? (
                  ...
                  >
                    <button
                      onClick={() =>
                        dispatch(duplicateLabelAction(oldVersionId, token))
                      }
                      style={{
                        marginTop: "20px",
                        padding: "5px 20px",
          ...
          <div>
            ...
            {allProjects.length < 1 && !labelRequest && (
