...
const DocumentInformation = () => {
  ...
  const [numOfPrint, setNumOfPrint] = useState(0);
  ...
  const handlePrintSuccess = () => {
    dispatch(saveToPrintAction({ ...data, action: "isPrinted" }, token)); // Adjust data and token variables as needed
    ...
    setData({ ...data, numOfPrint: documentInfo?.printLogs[0].numOfPrint });
    // }
    navigate(`/dashboard/templates`);
  };
  ...
  useEffect(() => {
    handleUDI({ projectInfo: documentInfo });
  ...
  return (
    <div className="container pt-3">
      ...
      >
        <Link
          style={{ height: "35px" }}
          to={`/dashboard/document-sizes/${documentId}`}
          className="label-info-link"
        >
          <ArrowBackIcon /> Back
        </Link>
        <button style={{ height: "35px" }} onClick={handlePrint}>
          <PrintIcon style={{ fontSize: "40px", color: "#8C2E2C" }} />
        </button>
      </div>
      <div>
        ...
        {size && (
          <h3 style={{ color: "", fontSize: "25px", textAlign: "center" }}>
            size: {size}mm
          </h3>
        )}
        ...
        <div className="document-content pt-4">
          {!documentByIdRequest ? (
            ...
            >
              {numOfPrint > 0 &&
                [...Array(parseInt(numOfPrint))].map((_, index) => {
                  const printCount =
                    documentInfo?.printCount -
                    documentInfo?.printLogs[0]?.numOfPrint +
                    index +
                    1;
                  return (
                    documentInfo &&
