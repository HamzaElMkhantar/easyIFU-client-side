...
import { approvedLabelsAction, drafLabelsAction } from '../../redux/actions/labelActions';
...
const ApprovedLabels = () => {
  ...
  const {approvedLabelsRequest, approvedLabelsSuccess, approvedLabelsFail, approvedLabel} = approvedLabels;

  const dispatch = useDispatch()
  useEffect(() => {
      dispatch(approvedLabelsAction({companyId, productId, createdBy: userId},token))
  }, [])
  useEffect(() => {
      if(approvedLabelsSuccess){
        setData(approvedLabel)
      }
      if(approvedLabelsFail && approvedLabelsFail.message != "doesn't exist any label!"){
          toast.error(`${approvedLabelsFail.message}`)
      }
    ...
    <div style={{height:'70vh', width:'100%', display:'flex'}}>
      ...
      <main style={{paddingTop:'0px', width:'100%'}}>
        ...
        <section className='container' style={{marginTop:'20px', padding:'10px'}}>
            <h6>Approved Label: {data.length} </h6>
            <div className="table-responsive">
                ...
                {!approvedLabelsRequest && data.length < 1 &&
                    <p style={{width:'100%', marginTop:'20px', display:'flex', justifyContent:'center'}}>
                        we didn't find any approved project!
                </p>
                }
            </div>
        </section>
