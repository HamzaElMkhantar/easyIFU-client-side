...
import {
  generateLotNumberAction,
  getLabelAction,
  saveLabelOrderAction,
...
import SuggestionsPopupComponent from "../../components/SuggestionsPopupComponent/SuggestionsPopupComponent";
...
const ReleasedProject = () => {
  ...
  const { getLabel, saveDocument, saveOrderLabel, generateLotNumber } = useSelector(
    (state) => state
  );
  const { getLabelRequest, getLabelSuccess, getLabelFail, label } = getLabel;
  const { generateLotsRequest, generateLotsSuccess, generateLotsFail, generateLotsData } = generateLotNumber;
  const {
    saveOrderLabelRequest,
    saveOrderLabelSuccess,
    savedOrderId,
    saveOrderLabelFail,
  } = saveOrderLabel;
  ...
  useEffect(() => {
    ...
    if (saveOrderLabelFail) {
      if(saveOrderLabelFail.message === "Lot Number exist"){
        let data = {
        ...
        dispatch(generateLotNumberAction(data, token))
  ...
  useEffect(() => {
    ...
    if (projectInfo && projectInfo.labelData) {
      ...
      if (haDateOfManufacture) {
        if (dynamicData.manufacturerDate !== "") {
          useByDateData =
            "(17)" +
            convertDateToYYMMDD(
              formatSumDateToYYYYMMDD(
                addDates(
                  dynamicData?.manufacturerDate,
                  projectInfo?.labelData?.useByDate
                )
              )
            );
          setUseByDateDataLabel(
            formatSumDateToYYYYMMDD(
              addDates(
                dynamicData?.manufacturerDate,
                projectInfo?.labelData?.useByDate
        ...
        if (dynamicData.expirationDate == "") {
          ...
          useByDateData =
            "(17)" + convertDateToYYMMDD(dynamicData?.expirationDate);
      ...
      let udiData =
        ...
        (haDateOfManufacture
          ? dynamicData.manufacturerDate !== ""
            ? "(11)" + convertDateToYYMMDD(dynamicData.manufacturerDate)
            : "yyyy-mm-dd"
          : "") +
        useByDateData +
        (hasLotNumber
          ? dynamicData.LotNumber !== ""
            ? "(10)" + dynamicData.LotNumber
            : "XXXXXXXX"
          : "") +
        (haSerialNumber
          ? dynamicData.serialNumber !== ""
            ? "(21)" + dynamicData.serialNumber
            : "XXXXXXXX"
          : "");
  ...
  // Function to handle suggestion click in the popup component
  const handleSuggestionClick = (suggestion) => {
    // setSelectedSuggestion(suggestion); // Set the selected suggestion in parent
    setDynamicData({ ...dynamicData, LotNumber: suggestion });
    setIsPopupOpen(false); // Close the popup
  };
  return (
    <div
      className="container label-information mb-5"
      style={{ padding: "0", height: "70vh", display: "flex" }}
    >
      ...
      {!getLabelRequest ? (
        <main style={{ marginTop: "30px", width: "100%" }}>
          <div style={{ display: "flex", alignItems: "center" }}>
            <Link to="/dashboard/templates" className="label-info-link mb-3">
              <ArrowBackIcon /> Back
            </Link>
            {!getLabelRequest && projectInfo && (
              <h6
                className="label-info-title mb-4"
          ...
          <div className="label-info-content-item row">
            ...
            <div className="col-md-4">
              ...
              {dateValidation ? (
                ...
                <p style={{ color: "red" }}>Date not Valid</p>
              )}
              <form
                style={{
                  backgroundColor: "#fff",
              ...
              >
                  ...
                  projectInfo?.labelData?.useByDate && (
                    ...
                    >
                      ...
                      <input
                        ...
                        value={dynamicData.useByDate}
                        name="useByDate"
                        onChange={(e) => handleManufacturerDateChange(e)}
                        style={{
                          borderBottom: "1px solid gray",
                          borderRadius: "0px",
                          padding: "0px 10px",
                ...
                {projectInfo?.labelData?.haSerialNumber && (
                    ...
                    >
                      ...
                      <input
                        ...
                        value={dynamicData.serialNumber}
                        onChange={(e) =>
                          setDynamicData({
                            ...dynamicData,
                ...
                {projectInfo?.labelData?.haDateOfManufacture && (
                    ...
                    >
                      ...
                      <input
                        placeholder="yyyy-mm-dd"
                        value={dynamicData.manufacturerDate}
              ...
              <div className="card mt-2">
                <div
                  className="form-group px-2 py-2"
                  style={{ display: "flex", flexDirection: "column" }}
                >
                  <label style={{ fontWeight: "500" }}>
                    how many label you want to print? : {dynamicData.printCount}
                  </label>
                  <input
                    placeholder="printCount"
                    type="number"
                    min="1"
                    value={dynamicData.printCount}
                    name="quantity"
                    onChange={(e) =>
                      setDynamicData({
                        ...dynamicData,
             ...
             {!projectInfo?.labelData?.haSerialNumber&&  <button style={{ width: "100%", marginTop: "10px", backgroundColor:'#068804' }} className="label-info-link" onClick={togglePopup}>
                Lot number Generator
              </button>}
              <button
                disabled={saveOrderLabelRequest ? true : false}
              ...
              <div className="mt-3" style={{ display: "", gridGap: "10px" }}>
                ...
                >
                  ...
                  />
                  Label Name: {projectInfo?.labelName}
                </p>
                <p
                  className="mb-1"
                  style={{
                ...
                {projectInfo?.labelDescription && (
                  ...
                  >
                    ...
                    />
                    Label description: {projectInfo?.labelDescription}
                  </p>
                )}
          ...
          {!projectInfo?.labelData?.haSerialNumber&& isPopupOpen && (
